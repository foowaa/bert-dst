...
logging.getLogger("pytorch_pretrained_bertt").setLevel(logging.CRITICAL)
data = JRCSDatasetReader.read('./new_data', dataset_fn='add_dataset.csv', split_rate=[0.30, 0.70, 0.00])
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id_intent, label_id_slot1, label_id_slot2, label_id_slot3, label_id_slot4, label_id_slot5, label_id_slot6):
        ...
        self.label_id_slot6 = label_id_slot6
        ...
        if example.label is not None:
            ...
            for i,e1 in enumerate(example.label):
                if i==0:
                    ...
                    tmp = [0 for _ in range(intents_num)]
                    for j in range(18):
                        if j in idx:
                            tmp[j] = 1
                    label_id_intent = tmp
...
def run_cls(training_examples, evaling_examples, label_for_list, test_examples=None):
    ...
    args.output_dir = './out/'+str(time.time())+'/'
    ...
    if args.do_train:
        ...
        all_label_ids7 = torch.tensor([f.label_id_slot6 for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids1,all_label_ids2,all_label_ids3, all_label_ids4,all_label_ids5,all_label_ids6,all_label_ids7)
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, label_ids1,label_ids2,label_ids3,label_ids4,label_ids5,label_ids6,label_ids7 = batch
                #TODO: forward error
                #FIXED
                #loss, _ = model(input_ids, segment_ids, input_mask, [label_ids1,label_ids2,label_ids3,label_ids4,label_ids5,label_ids6])
                loss,_ = model.forward(input_ids, segment_ids, input_mask, [label_ids1,label_ids2,label_ids3,label_ids4,label_ids5,label_ids6,label_ids7])
                if n_gpu > 1:
    ...
    if args.do_eval:
        ...
        all_label_ids7 = torch.tensor([f.label_id_slot6 for f in eval_features], dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids1,all_label_ids2,all_label_ids3,
                                   all_label_ids4,all_label_ids5,all_label_ids6,all_label_ids7)
        if args.local_rank == -1:
        ...
        time_s = []
        time_e = []
        for input_ids, input_mask, segment_ids, label_ids1, label_ids2,label_ids3,label_ids4,label_ids5,label_ids6,label_ids7 in eval_dataloader:
            ...
            label_ids7 = label_ids7.to(device)

            with torch.no_grad():
                tmp_eval_loss, logits = model(input_ids, segment_ids, input_mask,[label_ids1,label_ids2,label_ids3,label_ids4,label_ids5,label_ids6,label_ids7])
            ...
            label_ids7 = label_ids7.to('cpu').numpy()
            ...
            for i in range(task_num):
                ...
                if i==0:
                    ...
                    tmp_eval_accuracy[i] = accuracy_oridinary(logit_cpu, label_ids7)
                else:
                    pass
...
def train():
    ...
    ontologys = json.load(open("./new_data/ontology.json", encoding='utf-8'))
