...
import os
import sys
module_path = '/home/ubuntu/tianchunlin/distiller/'
...
data = JRCSDatasetReader.read('datas/gobot', dataset_fn='add_dataset.csv', split_rate=[0.10, 0.90, 0.00])
...
def run_cls(training_examples, evaling_examples, label_for_list, test_examples=None):
    ...
    args.output_dir = '/home/ubuntu/tianchunlin/v1_baoxian/out/'+str(time.time())+'/'
    ...
    args.compress = '/tmp/pycharm_project_569/configs/compress.yaml'
    ...
    if args.do_test:
        model_ = torch.load('/home/ubuntu/tianchunlin/v1_baoxian/model_c.pkl')
        model_ = model_.cuda()
        t1 = time.clock()
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #pdb.set_trace()
                compression_scheduler.on_minibatch_begin(epoch, minibatch_id=step,
                                                         minibatches_per_epoch=input_ids.shape[0])
                loss,_ = model.forward(input_ids, segment_ids, input_mask, [label_ids1,label_ids2,label_ids3,label_ids4,label_ids5,label_ids6])
                #pdb.set_trace()
                loss = compression_scheduler.before_backward_pass(epoch, minibatch_id=step,
                                                                  minibatches_per_epoch=input_ids.shape[0], loss=loss,
                ...
                #pdb.set_trace()
                compression_scheduler.on_minibatch_end(epoch, minibatch_id=step, minibatches_per_epoch=input_ids.shape[0])
            compression_scheduler.on_epoch_end(epoch)
    if args.do_eval:
        model = torch.load('/home/ubuntu/tianchunlin/v1_baoxian/model_c.pkl')
        model = model.cuda()
        time1 = time.clock()
    ...
    if args.saving:
            torch.save(model, '/home/ubuntu/tianchunlin/v1_baoxian/model_c.pkl')
