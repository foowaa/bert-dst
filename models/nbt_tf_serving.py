...
class NeuralBeliefTracker(TFModel):
    ...
    def __call__(self,slot,url, x_utter, x_sys_slot, x_sys_value, kp=1., prob=False):


        slot_to_english = {'身份': 'shenfen', '配合': 'peihe', '买过': 'maiguo', '了解': 'liaojie', '预约': 'yuyue', '意图': 'yitu'}

        data1 = {"x_utter": x_utter,"x_sys_slot":x_sys_slot,"x_sys_value":x_sys_value}

        url_slot=url+slot_to_english[slot]+':predict'

        param = {"inputs": data1}
        param = json.dumps(param, cls=NumpyEncoder)
        res = requests.post(url_slot, param)
        #res = requests.post('http://localhost:8501/v1/models/shenfen:predict', param)
        predict=eval(res.text)['outputs']#[0]
    ...
    def _gating_mechanism_slot(self):
        with tf.variable_scope("gating_mechanism_slot"):
            temp_sim = tf.multiply(self.c_slot, self.x_sys_slot)        # [ batch_size, word_dim ]
            gate_slot = tf.layers.dense(
                tf.reshape(temp_sim, [-1, self.word_dim]),
                self.filters_num,
                activation=tf.nn.sigmoid)
            return gate_slot      # [ batch_size, filters_num ]

    def _gating_mechanism_value(self):
        with tf.variable_scope("gating_mechanism_value"):
            list_sys_c_values_sim = []
            for i in range(self.label_num):
                temp_sim = tf.multiply(self.c_values[i, :], self.x_sys_value)
                this_m_v = tf.layers.dense(
                    tf.reshape(temp_sim, [-1, self.word_dim]),
                    self.filters_num,
                    activation=tf.nn.sigmoid)
                list_sys_c_values_sim.append(this_m_v)
            gate_value = tf.transpose(tf.stack(list_sys_c_values_sim), [1, 0, 2])
        return gate_value               # [batch_size, label_num, filters_num]

    def _context_modelling_slot(self, utter_encoded, gate_slot):
        with tf.variable_scope("context_modelling_slot"):
            gated_utterance = tf.multiply(utter_encoded, gate_slot)
    ...
    def _context_modelling_value(self, utter_encoded, gate_value):
        with tf.variable_scope("context_modelling_value"):
            ...
            for i in range(self.label_num):
                temp_gated_utterance = tf.multiply(gate_value[:, i, :], utter_encoded)    # element-wise multiplication
                list_gated_utterance.append(temp_gated_utterance)
            gated_utterance = tf.transpose(tf.stack(list_gated_utterance), [1, 0, 2])
        return gated_utterance          # [batch_size, label_num, n_filter]
    ...
    def _build_model(self):
        ...
        gate_slot = self._gating_mechanism_slot()           # [batch_size, filters_num]
        gate_value = self._gating_mechanism_value()         # [batch_size, label_num, filters_num]

        semantic_decoded_r_c = self._semantic_decoding(utter_encoded, candi_encoded)            # [batch_size, label_num, filters_num]
        context_decoded_m_slot = self._context_modelling_slot(utter_encoded, gate_slot)        # [batch_size, label_num, filters_num]
        context_decoded_m_value = self._context_modelling_value(utter_encoded, gate_value)      # [batch_size, label_num, filters_num]
        ...
        hidden_out_m_slot = tf.layers.dense(
            tf.reshape(context_decoded_m_slot, [-1, self.filters_num]),
            self.hidden_dim,
            activation=tf.nn.sigmoid)           # [batch_size * label_num, hidden_dim]

        hidden_out_m_value = tf.layers.dense(
            tf.reshape(context_decoded_m_value, [-1, self.filters_num]),
            self.hidden_dim,
            activation=tf.nn.sigmoid)           # [batch_size * label_num, hidden_dim]
        # hidden_out = tf.nn.dropout(hidden_out, self.keep_prob)

        logits = tf.layers.dense(hidden_out_d + hidden_out_m_slot + hidden_out_m_value, 1)
    ...
    def save(self, *args, **kwargs):
        ...
        #self.save_params()
        log.info('[saving model to {}]'.format(str(self.save_path)))
        builder = tf.saved_model.builder.SavedModelBuilder(str(self.save_path)+'/00000123')
        inputs = {
            # 你预测模型的时候需要传的参数，调用模型的时候，传参必须和这里一致
            "x_utter": tf.saved_model.utils.build_tensor_info(self.x_utter),
            "x_sys_slot": tf.saved_model.utils.build_tensor_info(self.x_sys_slot),
            "x_sys_value": tf.saved_model.utils.build_tensor_info(self.x_sys_value),
        }
        output = {"output": tf.saved_model.utils.build_tensor_info(self.y)}
